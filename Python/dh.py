...
class DiffieHellman(object):
	...
	def __readKeyPair(self,filename):
		"""
		Read a private and public key pair from filename
		"""
		myKeyFile = open(filename,'r')
		if not next(myKeyFile)[:-1] == 'private key':
			return False
		try:
			privKey = int(next(myKeyFile)[:-1])
		except ValueError as e:
			return False

		if not next(myKeyFile)[:-1] == '':
			return False
		if not next(myKeyFile)[:-1] == 'public key':
			return False

		try:
			pubKey = int(next(myKeyFile)[:-1])
		except ValueError as e:
			return False

		if not next(myKeyFile)[:-1] == '':
			return False
		if not next(myKeyFile)[:-1] == 'group':
			return False

		try:
			group = int(next(myKeyFile)[:-1])
		except ValueError as e:
			return False
	...
	def readKeyPair(self, filename):
		"""
		Load the priv/public key pair from filename
		"""
		with open(filename, 'r') as fp:
			data = json.load(fp)

		try:
			self.privateKey = int(data['privKey'])
			self.publicKey  = int(data['pubKey'])
			self.group	= int(data['group'])
		except ValueError as e:
			return False

		self.prime = self.getPrime(self.group)

		return True

	def storeKeypair(self, filename):
		"""
		Store priv/public keypair in filename
		"""
		data = {
	...
	def getFingerprint(self):
		try:
			_pubKeyBytes = self.publicKey.to_bytes(
					self.publicKey.bit_length() // 8 + 1, byteorder="big")
		except AttributeError:
			_pubKeyBytes = str(self.publicKey)

		s = hashlib.sha256()
		s.update(bytes(_pubKeyBytes))
		return s.digest()
